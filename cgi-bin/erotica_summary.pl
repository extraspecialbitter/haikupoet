...
$InputFile = "/usr/home/mena/public_html/haikupoet/erotica/log.html";

$log_page = "/usr/home/mena/public_html/haikupoet/erotica/summary.html";
...
# Process differently if referer info is present
 
   if ( $Line =~ /from/ )
   {
      $LogHost    = $Items[0]        ; # the logging host
      $LogDate    = $Items[4]        ; # the log date
      $LogTime    = $Items[6]        ; # the time of the log entry
   }
   else
   { 
      $LogHost    = $Items[0]        ; # the logging host
...
print OUT "    <center>";
print OUT "      <h1>Summarize Your Erotica in Low Places</h1>";
print OUT "    </center>";
print OUT "";
print OUT "    <hr>";
print OUT "";
